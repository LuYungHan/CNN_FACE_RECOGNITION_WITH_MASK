...
# +
@torch.no_grad()
def inference(net, img, file, threshold , model_align, cfg_align):
...
#     faceAlignModelHandler = FaceAlignModelHandler(model, 'cuda:0', cfg)
    faceAlignModelHandler = FaceAlignModelHandler(model_align, 'cuda:0', cfg_align)
...
# ################################################################################# get feature (network and models)
...
###################################################################################   face preprocessing models
    ...
    scene = 'non-mask'
#     model_category = 'face_detection'
#     model_detect_name =  model_conf[scene][model_category]

    logger.info('Start to load the face detection model...')
    # load model
    try:
        faceDetModelLoader = FaceDetModelLoader('models', 'face_detection', model_conf['non-mask']['face_detection'])
    except Exception as e:
        logger.error('Failed to parse model configuration file!')
        logger.error(e)
        sys.exit(-1)
    ...
    try:
        model_detect, cfg = faceDetModelLoader.load_model()
    except Exception as e:
        logger.error('Model loading failed!')
        logger.error(e)
        sys.exit(-1)
...
#     model_name =  model_conf[scene][model_category]
    ...
    # load model
    try:
        faceAlignModelLoader = FaceAlignModelLoader('models', 'face_alignment', model_conf['non-mask']['face_alignment'])
    except Exception as e:
        logger.error('Failed to parse model configuration file!')
        logger.error(e)
        sys.exit(-1)
    ...
    try:
        model_align, cfg_align = faceAlignModelLoader.load_model()
    except Exception as e:
        logger.error('Model loading failed!')
        logger.error(e)
        sys.exit(-1)
...
#     faceAlignModelHandler = FaceAlignModelHandler(model_align, 'cuda:0', cfg_align)
    print(type(model_align))
    print(type(cfg_align))
...
#     parser.add_argument('--weight', type=str, default='api_usage/work_dirs/ms1mv2_r50/model.pt')
    ...
    parser.add_argument('--net', type=str, default=net)-COLOR_BGR2RGB
    parser.add_argument('--threshold', type=int, default=1)    
    parser.add_argument('--model_align', type=str , default=model_align)
    parser.add_argument('--model_align_cfg', type=str, default=cfg_align)
    args = parser.parse_args()
    args = parser.parse_args(args=[])
    predict_is_the_same = inference(args.net, args.img, args.file, args.threshold,args.model_align,args.model_align_cfg)
