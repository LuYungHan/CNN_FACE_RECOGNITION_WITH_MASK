...
#             staff_id_face_path = file
            ...
            print(img_length)
            pic = cv2.imread(staff_id_face_path + '/' + '{}'.format(("%07d" % 0)) +'.jpg')
#             print(pic)
            if pic is None:
...
#             net.eval()
            ...
            for pic_num in range(1,img_length,1):
                pic = cv2.imread(staff_id_face_path + '/' + '{}'.format(("%07d" % pic_num)) +'.jpg')
                if pic is None:
                    pic = np.random.randint(0, 255, size=(112, 112, 3), dtype=np.uint8)
                else:
#                     pic = cv2.imread(pic)
...
#                 net.eval()
        ...
        #     print(all_feat.shape)
            ...
            print("save suc")
    print('total image number is ',total_image_number)
...
def merge_csv(path):
    ...
    for index, file in enumerate(files): 
        ...
        staff_id = file
        if not staff_id.startswith('.'):
            csv_path = dataset_path + '/' + staff_id + '/feat/' + '{}_feat.csv'.format(staff_id)
        #     print(csv_path)
            if (index < 1 ):
    ...
    save_feat_path = '.'
    all_face_feat_df = pd.DataFrame(dataseries)
    all_face_feat_df.to_csv(save_feat_path + '/' +'all_face_feat_0825-4.csv')
    print("save success all people feature !!!!")
...
#                     cv2.imwrite('api_usage/CCP_MI/{}'.format(staff_id) + '/crop_face/{}.jpg'.format("%07d" % image_label_name), cropped_image)
...
#     parser.add_argument('--img', type=str, default='api_usage/CCP_MI/10384/958FFE74E5401F07E0530B07A8C01186.jpg')
...
#                 print(image_path)
    ...
    parser.add_argument('--file', type=str, default='api_usage/CCP_MI')
    parser.add_argument('--net', type=str, default=net)
    args = parser.parse_args(args=[])
    feat2csv(args.file, args.net)
    merge_csv(args.file)