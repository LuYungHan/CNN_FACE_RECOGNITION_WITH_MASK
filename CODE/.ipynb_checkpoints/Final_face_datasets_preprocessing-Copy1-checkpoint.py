...
#             staff_id_face_path = file
            ...
            print(img_length)
            pic = cv2.imread(staff_id_face_path + '/{}'.format(("%d" % 0)) +'.jpg')
#             print(pic)
            if pic is None:
...
#             print(pic)
            ...
            all_feat = normalize(feat1)
            
            for pic_num in range(1,img_length,1):
                pic = cv2.imread(staff_id_face_path + '/' + '{}'.format(("%d" % pic_num)) +'.jpg')
                if pic is None:
                    pic = np.random.randint(0, 255, size=(112, 112, 3), dtype=np.uint8)
                else:
#                     pic = cv2.imread(pic)
...
def merge_csv(path):
    ...
    all_face_feat_df.to_csv(save_feat_path + '/' +'all_face_feat_0824.csv')
...
################################################################################## face detect
    ...
    for index, file in enumerate(files): 
        staff_id = file
        staff_id_path = path +'/'+ file
        staff_id_photo_path = staff_id_path +'/face'
