...
#     initialize_idx0 = []
#     initialize_idx1 = []
    initialize_idx0 = np.array(['staff_id'])
    initialize_idx1 = np.array(['label'])
#     analysis_dataseries = np.append(initialize_idx0,initialize_idx1,axis = 1)
    
#     print(analysis_dataseries)
    for index, file in enumerate(files): 
        staff_id = file
        staff_id_path = path +'/' + file
...
#             print(staff_id_face_path)
            staff_id = nmFolders[-2]
            print('input staff id: ',staff_id)
            img_num_files = os.listdir(staff_id_face_path)
#             print(img_num_files)
#             print(len(img_num_files))
#             pic = cv2.imread(staff_id_face_path + '/{}'.format(staff_id) + '_{}'.format(("%07d" % 0)) +'.jpg')
            img_length = 0 #'.jpg number initialize'
#             img_num_files = os.listdir(staff_id_face_path)
            
            
            for index1, img_id in enumerate(img_num_files): #
            ...
            for pic_num in range(0,img_length,1):
                pic = cv2.imread(staff_id_face_path + '/' + staff_id + '_{}'.format(("%07d" % pic_num)) +'.jpg')
                if pic is None:
                    pic = np.random.randint(0, 255, size=(112, 112, 3), dtype=np.uint8)
                else:
...
#################################################################################################### 歐式距離
                ...
                distances = np.sum(np.square(feat2 - compare_feat),axis = 1)**0.5
                idx = np.argmin(distances)
                min_distance = np.argmin(distances)
                dist_idx = distances[min_distance]
#                 print(dist_idx)
#################################################################################################### cosine
#                 cos = cosine_similarity(feat2, compare_feat)
#                 print(cos)
#                 idx = np.argmax(cos)
#                 max_cosine = np.argmin(cos)
#                 cos_idx = cos[max_cosine]
#################################################################################################### threshold
                threshold = 21.35
#################################################################################################### analysis result

                
#                 predict_result = (data[idx,2].split('.')[0])
#                 real_result = staff_id
#                 analysis_dataseries1 = []
#                 analysis_dataseries1 = [real_result,predict_result]
#                 analysis_dataseries = np.append(analysis_dataseries,analysis_dataseries1,axis = 0)
#                 print('This is :',data[idx,2])
                if dist_idx <= threshold : 
                    print('threshold,This is :',data[idx,2])
                    
                    label = (data[idx,2].split('.')[0])
                    
                    predict_result = np.array([(data[idx,2].split('.')[0])])
                    real_result = np.array([staff_id])
                    
                    initialize_idx0 = np.append(initialize_idx0,real_result,axis = 0)
                    initialize_idx1 = np.append(initialize_idx1,predict_result,axis = 0)
#                     analysis_dataseries1 = []
#                     analysis_dataseries1 = np.append(real_result,predict_result,axis = 1)
#                     analysis_dataseries = np.append(analysis_dataseries,analysis_dataseries1,axis = 0)
                    if staff_id != label:
                        error_img = error_img + 1
                        print('error############################',staff_id,label,error_img)
                else : 
                    print('unknown !!!')
                    
    initialize_idx0 = np.array([initialize_idx0]).T
    print(initialize_idx0)
    initialize_idx1 = np.array([initialize_idx1]).T
    print(initialize_idx1)
    initialize_idx0 = np.append(initialize_idx0,initialize_idx1,axis = 1)
    analysis_dataseries = initialize_idx0
    labeled_data = pd.DataFrame(analysis_dataseries)
    save_label_path = '.'
    labeled_data.to_csv(save_label_path + '/' +'label_result.csv')
    print("save suc")
    print("錯誤張數", error_img)
#         Ttime = str(datetime.now())
#         print(Ttime)
...
# ################################################################################# get feature 
    ...
    parser.add_argument('--net', type=str, default=net, help='backbone network')

#     parser.add_argument('--network', type=str, default='r50', help='backbone network')
#     parser.add_argument('--weight', type=str, default='api_usage/work_dirs/ms1mv2_r50/model.pt')
    parser.add_argument('--img', type=str,default='api_usage/CCP_MI_test/')
    parser.add_argument('--file', type=str, default='api_usage/all_face_feat_0822-2.csv')
